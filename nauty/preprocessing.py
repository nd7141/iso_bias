...
def clean_dataset(dataset, input_folder, output_folder, path_to_orbits, save_new_dataset = True):
    ...
    with open(graphs_fn) as f:
        ...
        for i, line in enumerate(f):
            ...
            if g1 != current_graph:  # assumes indicators are sorted
                ...
                G = nx.Graph()
                G.add_edges_from(edges)
                nx.set_edge_attributes(G, edge_labels, 'edge_label') if edge_labels_fn else None
                nx.set_edge_attributes(G, edge_attributes, 'edge_attribute') if edge_attributes_fn else None
                nx.set_node_attributes(G, node_labels, 'node_label') if node_labels_fn else None
                nx.set_node_attributes(G, node_attributes, 'node_attribute') if node_attributes_fn else None
    ...
    for graph_idx, graph in enumerate(new_graphs):
        if graph_idx in clean_graph_idx:
            ...
            # write graph edges
            for edge in sorted(graph_formatted.edges()):
            ...
            if edge_labels_fn:
                for edge in sorted(graph_formatted.edges()):
                    clean_edge_labels_f.write('{}\n'.format(graph_formatted.edges[(edge[0], edge[1])]['edge_label']))
                    clean_edge_labels_f.write('{}\n'.format(graph_formatted.edges[(edge[1], edge[0])]['edge_label']))

            # write edge attributes
            if edge_attributes_fn:
                for edge in sorted(graph_formatted.edges()):
                    clean_edge_attributes_f.write('{}\n'.format(graph_formatted.edges[(edge[0], edge[1])]['edge_attribute']))
                    clean_edge_attributes_f.write('{}\n'.format(graph_formatted.edges[(edge[1], edge[0])]['edge_attribute']))
...
def get_graph_labels(dataset):
    graph_labels_fn = f'datasets/{dataset}/{dataset}_graph_labels.txt'
    graph_labels = dict()
    with open(graph_labels_fn) as f:
        for i, label in enumerate(f):
            graph_labels[i + 1] = label.strip()
...
def verify_correctness_of_clean_datasets(dataset, path_to_old_datasets, path_to_new_datasets, path_to_orbits):
    ...
    graph_labels = get_graph_labels(dataset)
    ...
    for idx1, idx2 in enumerate(clean_graph_idx):
        ...
        try:
            ...
            # the following is double check, just to compare node by node and edge by edge, that the graphs are the same
            ...
            if node_labels_fn:
                for node in G1:
                    node_label1 = G1.nodes()[node]['node_label']
                    node_label2 = G2.nodes()[node]['node_label']
                    assert node_label1 == node_label2

            # check that node attributes are the same
            if node_attributes_fn:
                for node in G1:
                    node_attribute1 = G1.nodes()[node]['node_attribute']
                    node_attribute2 = G2.nodes()[node]['node_attribute']
                    assert node_attribute1 == node_attribute2

            # check that edge labels are the same
            if edge_labels_fn:
                for edge in G1.edges():
                    edge_label1 = G1.edges()[edge]['edge_label']
                    edge_label2 = G2.edges()[edge]['edge_label']
                    assert edge_label1 == edge_label2

            # check that edge attributes are the same
            if edge_attributes_fn:
                for edge in G1.edges():
                    edge_attribute1 = G1.edges()[edge]['edge_attribute']
                    edge_attribute2 = G2.edges()[edge]['edge_attribute']
                    assert edge_attribute1 == edge_attribute2

            print('Passed', idx1, idx2)
    print('New clean dataset is correct')

if __name__ == "__main__":

    dataset = 'BZR_MD'

    output_folder = f'clean_datasets/{dataset}/'
    ...
    path_to_old_datasets = 'datasets/'
    path_to_new_datasets = 'clean_datasets/'
    path_to_orbits = 'orbits/no_labels/'
    ...
    #  'TRIANGLES',
    #  'TWITTER-Real-Graph-Partia']
>>>>>>> Add verification of correctness of new datasets:preprocessing.py
